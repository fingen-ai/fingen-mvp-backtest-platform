...
public class OMSImplTest {
    ...
    public void testAddAndRetrieveOrder() throws IOException {
        ...
        nos.transactTime = String.valueOf(System.nanoTime());  // Set the transaction time
        nos.orderQty = 10;  // Ensure quantity is set
        nos.price = 250.0;  // Ensure price is set
        nos.side = String.valueOf(BuySell.buy);  // Set the side to avoid null
        nos.clOrdID = "Order123";  // Ensure client order ID is set
        nos.ordType = String.valueOf(OrderType.limit);  // Set the order type to avoid null
