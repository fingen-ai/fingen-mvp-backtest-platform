...
public class PublisherWrapper<I extends PublisherHandler> implements Runnable, Closeable {
    ...
    public PublisherWrapper(String inputPath, String outputPath, I serviceImpl) {
        // Directly specify the class when creating the method writer
        outputQueue = SingleChronicleQueueBuilder.binary(outputPath).build();
        serviceOut = outputQueue.acquireAppender().methodWriter(PublisherPub.class);
